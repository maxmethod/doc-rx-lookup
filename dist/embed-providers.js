...
//   window.PROV_CONFIG = { initialProviders: '{{ contact.custom.providers_json }}' }
//   <div id="provider-lookup-widget" data-initial-providers="{{ contact.custom.providers_json }}">
function readInitialSeed() {
  ...
  let s = clean(cfg.initialProviders);
  if (s) return s;
  const el = document.getElementById('provider-lookup-widget');
  if (el) s = clean(el.getAttribute('data-initial-providers'));
  return s;
}
