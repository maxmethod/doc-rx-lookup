...
(function () {
  ...
  * { box-sizing: border-box; }
  ...
  .result-item:hover, .result-item.active { background: #eef3fa; }
  ...
  <div class="section" id="medications-section">
    <div class="section-header">
      ...
      <span class="section-count" id="med-count">0 of 10</span>
    </div>
  ...
  <div class="section" id="providers-section">
    <div class="section-header">
      ...
      <span class="section-count" id="prov-count">0 of 5</span>
    </div>
  ...
  <div class="submit-section">
    <input type="hidden" id="medications_json" name="medications_json">
    <input type="hidden" id="medications_summary" name="medications_summary">
    <input type="hidden" id="providers_json" name="providers_json">
    <input type="hidden" id="providers_summary" name="providers_summary">

    <button type="button" class="primary" id="preview-btn" style="width:100%; padding:12px;">Preview data that will submit</button>
...
const ZIP_DATASET_VERSION = 'v1.0.2';
...
document.getElementById('med-manual-add').onclick = () => {
  ...
  state.medications.push(entry);
  ['med-manual-name','med-manual-strength','med-manual-form'].forEach(id => document.getElementById(id).value = '');
  renderMedications();
  resetMedSearch();
};
    ...
    return `
      <div class="item-card ${m.source === 'manual' ? 'manual' : ''}">
        <div class="item-card-main">
          ...
          <div class="item-card-meta">${escapeHtml(m.frequency)}</div>
        </div>
...
document.getElementById('preview-btn').onclick = () => {
  ...
  out.textContent = [
    '===== medications_json =====',
    buildMedicationsJson(),
    '',
    '===== medications_summary =====',
    buildMedicationsSummary(),
    '',
    '===== providers_json =====',
    buildProvidersJson(),
    '',
...
document.getElementById('debug-toggle').onclick = () => {
  document.getElementById('debug-output').classList.toggle('visible');
};
