...
// type="application/json" and id="coverage-lookup-widget-seed" whose text is
// {{ contact.custom.current_coverage_json }}, placed BEFORE the embed script element.
...
function readInitialSeed() {
  ...
  const seedTag = document.getElementById('coverage-lookup-widget-seed');
  if (seedTag) {
  ...
  return clean(cfg.initialCoverage);
}
