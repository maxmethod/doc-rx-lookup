...
//   window.COVERAGE_CONFIG = { initialCoverage: '{{ contact.custom.current_coverage_json }}' }
//   <div id="coverage-lookup-widget" data-initial-coverage="{{ contact.custom.current_coverage_json }}">
function readInitialSeed() {
  ...
  const cfg = (typeof window !== 'undefined' && window.COVERAGE_CONFIG) || {};
  let s = clean(cfg.initialCoverage);
  if (s) return s;
  const el = document.getElementById('coverage-lookup-widget');
  if (el) s = clean(el.getAttribute('data-initial-coverage'));
  return s;
}

function hydrateFromField() {
  try {
    const raw = readFieldValue(FIELD_KEYS.current_coverage_json) || readInitialSeed();
    if (!raw) return;
