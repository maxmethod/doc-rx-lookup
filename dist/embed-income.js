...
(function () {
  ...
  <div class="section" id="income-section">
    ...
    <div class="add-box" id="inc-add-box">
      ...
      <div class="warn-banner" id="inc-warn">Enter an amount before adding.</div>
      ...
      <div class="field-group">
        ...
        <div>
          <label for="inc-source">Source / Employer (optional)</label>
          <input type="text" id="inc-source" placeholder="e.g. Acme Corp, rental property" autocomplete="off">
        </div>
      </div>
...
// Amount is the only required field; Source / Employer is optional.
els.amount.addEventListener('input', () => {
  if (Number(els.amount.value) > 0) els.warn.classList.remove('visible');
});
...
document.getElementById('inc-add').onclick = () => {
  ...
  state.income.push({
    ...
    // Source / Employer is optional; for an "Other" income type the user can
    // describe where it comes from here. May be blank.
    source_name: els.source.value.trim(),
    amount,
...
function renderIncome() {
  ...
  list.innerHTML = state.income.map(r => {
    ...
    // Source is optional: fall back to the income type as the card title, and
    // only repeat the type in the meta line when a source is actually shown.
    const name = r.source_name || r.income_type;
    const meta = [];
    if (r.source_name) meta.push(escapeHtml(r.income_type));
    ...
    return `
      <div class="item-card">
        <div class="item-card-main">
          ...
          ${metaStr ? `<div class="item-card-meta">${metaStr}</div>` : ''}
        </div>
...
function buildIncomeSummary() {
  ...
  for (const r of state.income) {
    // With no source, lead with the income type; otherwise "Source (Type)".
    const label = r.source_name ? `${r.source_name} (${r.income_type})` : r.income_type;
    let line = `• ${label} — ${fmtMoney(r.amount)} ${r.frequency.toLowerCase()}`;
